...
- **Products** — содержит информацию по товарам (SKU, категория, предмет, бренд, цена, продажи, выручка, средний % выкупа, страна).
- **Dynamic** — содержит динамику финансовых метрик по брендам за последние полгода.
...
1. **Динамика за полгода (по брендам)**
...
2. **Детализация за месяц (включая Топ-10 проблемных сегментов)**
...
Поле **"Рекомендации"** вычисляется автоматически для каждого сегмента и отдельного товара (SKU) на основании агрегированных показателей. Возможные значения:
...
Логика полностью соответствует бизнес-требованиям, описанным в [кейсе](Кейс_1_Финансовые_показатели.pdf), и применяется как к отдельным товарам, так и к агрегированным сегментам (брендам, категориям и т.д.).

---

## Визуальные и технические особенности

- В ряде случаев визуализации были адаптированы по сравнению с исходным канвасом ([Canvas.jpg](Canvas.jpg)) из-за ограничений Yandex Datalens:
